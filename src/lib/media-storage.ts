...
export function formatUploadError(err: unknown): string {
  ...
  if (/public access on a private store/i.test(msg)) {
    return 'Your Blob store is private but the app uploaded as public. Create a new public store or we can use private mode — see README.';
  }
  if (/access must be ["']public["']/i.test(msg)) {
    return "Your Blob store is public — redeploy the latest app (uploads use public access).";
  }
